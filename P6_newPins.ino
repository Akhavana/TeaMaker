...
int brewTime = 17; //seconds
int dispenseTime = 10; //seconds
int waterHI = 85; //celsius
int waterLO = waterHI - 1; // hysterisis
int teaMassTarget = 2; //grams
int waterMassTarget = 50; //grams
int cupsOfTea = 1;
...
bool waterMassChecker() { // measures if correct mass of water has been dispensed
  if (checkScale() > 200*waterMassTarget) { //50g
    return true;
  }
  else {
    return false;
...
bool teaMassChecker() { // measures if correct mass of tea leaves has been dispensed
    if (checkScale() > 200*teaMassTarget) { //2g
    return true;
  }
  else {
    return false;
